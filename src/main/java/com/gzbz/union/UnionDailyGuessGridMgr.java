...
public class UnionDailyGuessGridMgr {
   private int row;
   private int col;
   private List<UnionDailyGuessGridNode> list = new ArrayList();
   ...
   public void init(int row, int col) {
      if (this.list.size() != row * col || this.row != row || this.col != col) {
         ...
         for(int i = 0; i < row; ++i) {
            for(int j = 0; j < col; ++j) {
               UnionDailyGuessGridNode gridNode = new UnionDailyGuessGridNode(j, i);
   ...
   public Set<Integer> getNear(int grid) {
      ...
      if (grid >= 0 && grid <= this.list.size()) {
         UnionDailyGuessGridNode gridNode = (UnionDailyGuessGridNode)this.list.get(grid - 1);
