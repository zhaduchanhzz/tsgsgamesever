...
public class GamePlayer extends GameMgr {
    ...
    public <T> List<T> getMgrParts(Class<T> parentCls) {
        ...
        List<Class<?>> partClasses = scanAnnotation.partMap.get(this.getClass());
        ...
        for (Class<?> subClass : partClasses) {
            if (parentCls.isAssignableFrom(subClass)) {
                T part = parentCls.cast(this.getMgrPart(subClass));
                mgrParts.add(part);
            }
        }
        ...
        if (cost > 5L) {
            logger.info(
                    "Initialized sub-parts of {} for player {} in {} ms",
                    parentCls.getSimpleName(),
                    this.getPlayerId(),
                    cost
            );
        }
...
//   }
    ...
    public void logout() {
        ...
        boolean needCreateLogoutLog = true;

        // Ensure basic player info
        if (StringUtils.isEmpty(playerDao.account)) {
            playerDao.account = this.account;
            needCreateLogoutLog = false;
        }
        ...
        playerDao.logoutTime = logoutTime;
        playerDao.updateOp();

        long currentOnlineMillis = 0L;

        if (playerDao.lastLoginTime > 0L) {
            currentOnlineMillis = logoutTime - playerDao.lastLoginTime;
            handleOnlineTime(playerDao, currentOnlineMillis);
            afterLogoutHandle(logoutTime);
        }

        if (needCreateLogoutLog) {
            handleLogoutLog(playerDao, currentOnlineMillis);
        }

        // Execute logout handlers of all parts
        executePartLogoutHandlers();

        gameCachePool.setOpenCheckExpire(true);
        isLogin = false;

        // Refresh public DAO snapshot
        playerPublicDao = new PlayerPublicDao(playerDao);
    }

    private void executePartLogoutHandlers() {
        checkInit_PARTS_SORTED_LOGIN_PRIORITY();

        for (PlayerPart part : PARTS_SORTED_LOGIN_PRIORITY) {
            try {
                part.logoutHandle();
            } catch (Exception e) {
                logger.error("Error during logoutHandle for part: {}",
                        part.getClass().getSimpleName(), e);
            }
        }
    }

    private void handleLogoutLog(PlayerDao playerDao, long onlineMillis) {
        operationMgr.changeHeadOldLog(this);

        String clientIp = getLogClientIp();
        if (clientIp != null && clientIp.startsWith("null:")) {
            logger.warn("Cannot obtain IP for player {}-{}, raw value: {}",
                    playerId,
                    playerDao.playerName,
                    clientIp.split(":")[1]);
            return;
        }

        operationMgr.addLogoutLog(this, onlineMillis, clientIp);
    }

    private void handleOnlineTime(PlayerDao playerDao, long onlineMillis) {
        PlayerExtend playerExtend = (PlayerExtend) getPlayerExtend(13);
        if (playerExtend != null) {
            playerExtend.todayOnlineTime += onlineMillis;
            playerExtend.totalOnlineTime += onlineMillis;
            updatePlayerExtend(13);
        }

        // Guest device tracking
        if (isGuest && !StringUtils.isEmpty(devUID)) {
            WorldMgr worldMgr = ApplicationContextProvider
                    .getContext()
                    .getBean(WorldMgr.class);

            worldMgr.pushTask(() ->
                    worldMgr.recordDeviceOnlineTime(devUID, onlineMillis));
        }

        // Client tracking
        ClientTrackingPart trackingPart =
                (ClientTrackingPart) getMgrPart(ClientTrackingPart.class);

        if (trackingPart != null) {
            trackingPart.removeTrack(
                    ClientTrackingEnum.SP_LAST_ONLINE_TIME,
                    ClientTrackingEnum.SP_LAST_ONLINE_TIME.name());

            long seconds = onlineMillis / 1000L;

            trackingPart.clientTrack(
                    ClientTrackingEnum.SP_LAST_ONLINE_TIME,
                    ClientTrackingEnum.SP_LAST_ONLINE_TIME.name(),
                    "type=event_track",
                    "event=Time_Spent",
...
//   }
    ...
    public void msgCreateRole(LoginMsg.C2S_CreateRole_1002 msg, String source) {
        ...
        // Prevent duplicate role creation requests
        if (createState != null && createState.get()) {
            return;
        }
        ...
        if (!checkPlayerNameLength(playerName)) {
            return;
        }
        ...
        createState = new AtomicBoolean(true);
        ...
        worldMgr.pushTask(() -> {
            ...
            if (worldMgr.playerNameIsExist(finalPlayerName)) {
                failure(13003);
            ...
            this.pushTask(() -> {
                ...
                // Validate playerId
                if (playerDao.playerId < 20000) {
                    logger.error("Invalid playerId detected: {}", playerDao.playerId);
                    createState.set(false);
                    return;
                }
                ...
                if (msg.hasRecommendCode()) {
                    RecommendPart recommendPart =
                            (RecommendPart) getMgrPart(RecommendPart.class);

                    if (recommendPart == null ||
                            !recommendPart.invited(msg.getRecommendCode())) {

                        failure(20007);

                        // Rollback reserved player name
                        worldMgr.pushTask(() ->
                                worldMgr.getPlayerName().remove(finalPlayerName));
                ...
                // Player already has a created role
                if (!StringUtils.isEmpty(playerDao.playerName)) {
                    login(false);
                    createState.set(false);
                    return;
                ...
                // Initialize base player information
                playerDao.serverId = serverId;
                playerDao.account = account;
                playerDao.sex = (byte) msg.getSex();
                playerDao.playerName = finalPlayerName;
                playerDao.headFrame = 30001;
                playerDao.head = (msg.getSex() == 1) ? 1 : 2;
                playerDao.birthTime = System.currentTimeMillis();
                playerDao.setProperty(PlayerDefine.PLAYER_LV, 1L);
                ...
                playerDao.monarchId =
                        configManager.getInt("DefaultHeroMonarchImage");

                gameCachePool.updateDao(playerDao, true);

                // Grant default hero
                int defaultHeroId =
                        configManager.getInt("playerDefaultHero");

                addResource(3, defaultHeroId, 1,
                        PlayerMsg.ShowType.SHOW_TYPE_NOT_SHOW,
                        4, 400, 0, 0, "");

                // Special rewards for JP server
                if ("jp".equals(worldMgr.country)) {
                    String configStr =
                            ApplicationContextProvider.getConfigString(
                                    "createRolesitem_jp", "1|9|44");

                    addResource(
                            ResourceModel.buildResources(configStr),
                            PlayerMsg.ShowType.SHOW_TYPE_NOT_SHOW,
                            13, 1315, 0, 0, ""
                    );
                }
                ...
                // Initialize adventure tasks
                AdventureTaskPart adventureTaskPart =
                        (AdventureTaskPart) getMgrPart(AdventureTaskPart.class);

                if (adventureTaskPart != null) {
                    adventureTaskPart.initTask();
                }

                isLogin = true;

                registerLog(finalPlayerName, account);
                ...
                if (clientIp != null && clientIp.startsWith("null:")) {
                    logger.warn(
                            "Create role log skipped - unable to obtain IP for player {}-{}, raw value: {}",
                            playerId,
                            playerDao.playerName,
                            clientIp.split(":")[1]
                    );
                } else {
                    operationMgr.addCreateUserNewLog(this, clientIp);
                }

                operationMgr.addCreateUserOldLog(this);

                // iOS GM special configuration
                if (ApplicationContextProvider.getConfigInt("iosGmOpen", 0) > 0) {

                    ChatMgr chatMgr =
                            ApplicationContextProvider.getContext()
                                    .getBean(ChatMgr.class);

                    chatMgr.gmAddLvByExp(
                            this,
                            ApplicationContextProvider.getConfigInt("iosPlayerLv", 100),
                            7, 7600, 0, 0, "ios"
                    );

                    String heroStr =
                            ApplicationContextProvider.getConfigString("iosHero", null);

                    if (heroStr != null) {
                        HeroBagPart heroBagPart =
                                (HeroBagPart) getMgrPart(HeroBagPart.class);

                        int heroLv =
                                ApplicationContextProvider.getConfigInt("iosHeroLv", 255);
                        int heroStar =
                                ApplicationContextProvider.getConfigInt("iosHeroStar", 10);

                        for (String hero : heroStr.split("\\|")) {
                            int heroId = Integer.parseInt(hero);
                            heroBagPart.addHeroForAssign(
                                    heroId, 1, heroLv, heroStar,
                                    49, 7600, 0, 0, "ios"
                            );
                        }
                    }
                    ...
                    List<ResourceModel> addList = new ArrayList<>();
                    addList.add(new ResourceModel(
                            1,
                            PlayerDefine.PLAYER_GOLD,
                            ApplicationContextProvider.getConfigInt("iosPlayerGold", 100000)
                    ));
                    addList.add(new ResourceModel(
                            1,
                            PlayerDefine.PLAYER_COPPER,
                            ApplicationContextProvider.getConfigInt("iosPlayerMoney", 100000)
                    ));
                    ...
                    CrossMsg.S2CR_PlayerBaseData_4202.Builder builder =
                            CrossMsg.S2CR_PlayerBaseData_4202.newBuilder();
                    ...
                    builder.setSex(playerDao.sex);
                    builder.setLv(playerDao.lv);
                    builder.setCombatPower(getPlayerCombatPower());
                    builder.setHeroNum(
                            getBattleTeam(CommonMsg.HeroState.HERO_STATE_ARRAYING, null)
                                    .getEntityMap().size());
                    builder.setHeadId(playerDao.head);
                    builder.setHeadFrame(playerDao.headFrame);
                    builder.setPrestige(playerDao.prestige);
                    ...
                    if (playerDao.unionId > 0) {
                        UnionMgrParent unionMgrParent =
                                ApplicationContextProvider.getContext()
                                        .getBean(UnionMgrParent.class);

                        UnionDao unionDao =
                                unionMgrParent
                                        .getUnionMgr(playerDao.unionId)
                                        .getUnion();
                ...
                // Handle comeback activity registration
                ActivityComeBackPart comeBackPart =
                        (ActivityComeBackPart) getMgrPart(ActivityComeBackPart.class);

                if (comeBackPart != null &&
                        !comeBackPart.isServerActivityInvalid()) {
