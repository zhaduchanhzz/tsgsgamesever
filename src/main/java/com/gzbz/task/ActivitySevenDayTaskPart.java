...
public class ActivitySevenDayTaskPart extends AbstractActivityTaskPart {
   public void trigger() {
   ...
   private List<SevenDaysTaskModel> getCurTaskList(ActivityOpenTimeModel activityOpenTimeModel, int day) {
      ...
      if (activityOpenTimeModel.getActivityId() == 1131) {
         Map<Integer, TaskData> taskDataMap = this.getTasksMap();
         Set<Integer> curLoginDayTask = new HashSet();

         for(Integer taskId : taskDataMap.keySet()) {
            SevenDaysTaskModel taskModel = (SevenDaysTaskModel)ApplicationContextProvider.getModelPoolEntity("sevenDaysTasks", taskId);
            if (taskModel != null) {
               if (day > 1) {
                  if (taskModel.getTargetType() != 101) {
                     continue;
                  }
               } else if (taskModel.getTargetType() == 101 && taskModel.getTime() != day) {
                  continue;
               }

               curLoginDayTask.add(taskModel.getTime());
            }
         }

         for(SevenDaysTaskModel taskModel : taskModels) {
            if (!curLoginDayTask.contains(taskModel.getTime())) {
               if (day > 1) {
                  if (taskModel.getTargetType() != 101 || taskModel.getTime() > day) {
                     continue;
                  }
               } else if (taskModel.getTargetType() == 101 && taskModel.getTime() != day) {
                  continue;
   ...
   public void sendTaskList(List<TaskData> taskDataList) {
      ActivityMsg.S2C_SevenTasksNotify_6762.Builder msg = ActivityMsg.S2C_SevenTasksNotify_6762.newBuilder();
   ...
   public TaskDefine.TaskModule getModule() {
      return TaskDefine.TaskModule.MODULE_ACTIVITY_SEVEN;
   }
   ...
   protected void sendCommitTaskResult(int taskId, long completeTime) {
      ...
      ActivityMsg.S2C_SevenTaskCommitTask_6756.Builder resp = ActivityMsg.S2C_SevenTaskCommitTask_6756.newBuilder();
      resp.setTaskId(taskId);
      resp.setTaskPoint(playerActivityDao.sevenTaskPoint);
   ...
   public void sendInfo() {
      ActivityMsg.S2C_SevenTaskActivityNotify_6748.Builder msg = ActivityMsg.S2C_SevenTaskActivityNotify_6748.newBuilder();
      ...
      msg.setDay(day);
      PlayerActivityDao playerActivityDao = (PlayerActivityDao)this.player.getData("tb_player_activity", this.player.getPlayerId());
      msg.addAllGotBoxIds(playerActivityDao.boxReward);
      msg.setTaskPoint(playerActivityDao.sevenTaskPoint);
      this.player.sendMsg(msg.build());
   }
   ...
   public void clearTasks() {
      if (this.getPlayerActivityId() > 0 && this.getPlayerActivityId() != 1131) {
