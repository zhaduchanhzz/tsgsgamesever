...
public class EffectSputtering extends MagicFunc {
   public boolean process(Action action, Entity source, Entity target, List<Entity> targets, SkillEffectModel model, Event event) {
      long totalDamageValue = (Long)action.getBlackboard().getSkillParam(33, 0L);
      if (totalDamageValue == 0L) {
         return false;
      } else {
         long newDamage = totalDamageValue * (long)model.getParam("damageRate") / 10000L;
         if (newDamage == 0L) {
            return false;
         } else {
            boolean isAlive = target.isAlive();
            target.recordHp(source);
            EffectUtil.doShield(action, source, target, newDamage, model, false, false, event, (byte)0);
