...
public class TransferTargetHpToSelf extends EffectFunc {
   ...
   public boolean process(Action action, Entity source, Entity target, List<Entity> targets, SkillEffectModel model, Event event) {
      if (!target.getStateManager().checkState((short)22) && !target.getStateManager().checkState((short)25)) {
         long lostValue = (long)((float)model.getParam("rate") / 10000.0F * (float)target.getBaseProperty(99));
         if (lostValue == 0L) {
            return false;
         } else {
            if (model.getFuncParam().containsKey("limitRate") && model.getFuncParam().containsKey("propertyId")) {
               long limitValue = (long)((float)(source.getBaseProperty((Integer)model.getFuncParam().get("propertyId")) * (long)(Integer)model.getFuncParam().get("limitRate")) / 10000.0F);
            ...
            long resumeHp = (long)((float)lostValue * cureRate);
