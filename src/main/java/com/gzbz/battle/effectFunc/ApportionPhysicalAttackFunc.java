...
public class ApportionPhysicalAttackFunc extends DamageFunc {
   ...
   public boolean process(Action action, Entity source, Entity target, List<Entity> targets, SkillEffectModel model, Event event) {
      if (target.isDead()) {
         ...
         long damage = getPhyAttack(action, attack, source, target, (float)totalRate / 10000.0F / (float)targets.size());
