...
@ModelAnno(
   file = "eventHeroBossReward"
)
public class EventHeroBossRewardModel extends BaseModel {
   @ModelColumnAnno(
      columnName = "id",
      isPrimaty = true,
      comment = "id"
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, List<EventHeroBossRewardModel>> tempMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         EventHeroBossRewardModel model = (EventHeroBossRewardModel)entry.getValue();
         if (tempMap.containsKey(model.getActivityId())) {
            ((List)tempMap.get(model.getActivityId())).add(model);
         } else {
            List<EventHeroBossRewardModel> list = new ArrayList();
            list.add(model);
            tempMap.put(model.getActivityId(), list);
         }
      }

      pool.putMap("customEventHeroBossReward", tempMap);
   }
}
