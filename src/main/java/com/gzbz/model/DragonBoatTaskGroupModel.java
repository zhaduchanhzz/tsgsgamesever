...
@ModelAnno(
   file = "dragonBoatTaskGroup"
)
public class DragonBoatTaskGroupModel extends BaseModel {
   @ModelColumnAnno(
      columnName = "id",
      isPrimaty = true,
      comment = "ID"
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, List<DragonBoatTaskGroupModel>> tempMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         DragonBoatTaskGroupModel model = (DragonBoatTaskGroupModel)entry.getValue();
         if (tempMap.containsKey(model.getTaskGroup())) {
            ((List)tempMap.get(model.getTaskGroup())).add(model);
         } else {
            List<DragonBoatTaskGroupModel> list = new ArrayList();
            list.add(model);
            tempMap.put(model.getTaskGroup(), list);
         }
      }

      pool.putMap("customDragonBoatTask", tempMap);
   }
}
