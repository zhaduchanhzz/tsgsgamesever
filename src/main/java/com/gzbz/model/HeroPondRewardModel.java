...
public class HeroPondRewardModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true,
      comment = "序号"
   )
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, ConcurrentHashMap<Integer, List<HeroPondRewardModel>>> tempHashMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         HeroPondRewardModel model = (HeroPondRewardModel)entry.getValue();
         if (model.getType() == 1 && model.getOpenDays() > 0) {
            ((List)((ConcurrentHashMap)tempHashMap.computeIfAbsent(model.getActId(), (v) -> new ConcurrentHashMap())).computeIfAbsent(model.getOpenDays(), (v) -> new ArrayList())).add(model);
         }
      }

      pool.putMap("customHeroPondReward", tempHashMap);
   }
