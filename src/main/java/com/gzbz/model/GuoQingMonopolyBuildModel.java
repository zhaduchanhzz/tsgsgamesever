...
@ModelAnno(
   file = "GuoQingMonopolyBuild"
)
public class GuoQingMonopolyBuildModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true
   )
   private int id;
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, TreeMap<Integer, GuoQingMonopolyBuildModel>> tempHashMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         GuoQingMonopolyBuildModel model = (GuoQingMonopolyBuildModel)entry.getValue();
         ((TreeMap)tempHashMap.computeIfAbsent(model.buildId, (v) -> new TreeMap())).put(model.buildLv, model);
      }

      pool.putMap("cusTomGuoQingMonopolyBuild", tempHashMap);
   }
