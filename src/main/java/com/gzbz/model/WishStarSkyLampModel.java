...
@ModelAnno(
   file = "WishstarSkyLamp"
)
public class WishStarSkyLampModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true,
      comment = "序号"
   )
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, List<WishStarSkyLampModel>> stage_List = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         WishStarSkyLampModel value = (WishStarSkyLampModel)entry.getValue();
         ((List)stage_List.computeIfAbsent(value.getStage(), (k) -> new ArrayList())).add(value);
      }

      pool.putMap("CustomWishstarSkyLamp", stage_List);
   }
