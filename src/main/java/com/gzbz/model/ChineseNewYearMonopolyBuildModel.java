...
@ModelAnno(
   file = "ChunJieMonopolyBuild"
)
public class ChineseNewYearMonopolyBuildModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true
   )
   private int id;
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, TreeMap<Integer, ChineseNewYearMonopolyBuildModel>> tempHashMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         ChineseNewYearMonopolyBuildModel model = (ChineseNewYearMonopolyBuildModel)entry.getValue();
         ((TreeMap)tempHashMap.computeIfAbsent(model.buildId, (v) -> new TreeMap())).put(model.buildLv, model);
      }

      pool.putMap("cusTomChunJieMonopolyBuild", tempHashMap);
   }
