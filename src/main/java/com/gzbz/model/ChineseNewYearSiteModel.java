...
public class ChineseNewYearSiteModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true,
      columnName = "id"
   )
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, ConcurrentHashMap<Integer, ChineseNewYearSiteModel>> tempMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         ChineseNewYearSiteModel model = (ChineseNewYearSiteModel)entry.getValue();
         ((ConcurrentHashMap)tempMap.computeIfAbsent(model.getType(), (v) -> new ConcurrentHashMap())).put(model.getLevel(), model);
      }

      pool.putMap("customChunJieSite", tempMap);
   }
