...
public class FirstYearEventModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true
   )
   private int id;
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, TreeMap<Integer, FirstYearEventModel>> tempHashMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         FirstYearEventModel model = (FirstYearEventModel)entry.getValue();
         ((TreeMap)tempHashMap.computeIfAbsent(model.eventId, (v) -> new TreeMap())).put(model.id, model);
      }

      pool.putMap("customFristYearEvent", tempHashMap);
   }
