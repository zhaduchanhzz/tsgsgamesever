...
@ModelAnno(
   file = "HJWeekMonopolyBuild"
)
public class GoldWeekMonopolyBuildModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true
   )
   private int id;
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, TreeMap<Integer, GoldWeekMonopolyBuildModel>> tempHashMap = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         GoldWeekMonopolyBuildModel model = (GoldWeekMonopolyBuildModel)entry.getValue();
         ((TreeMap)tempHashMap.computeIfAbsent(model.buildId, (v) -> new TreeMap())).put(model.buildLv, model);
      }

      pool.putMap("cusTomHJWeekMonopolyBuild", tempHashMap);
   }
