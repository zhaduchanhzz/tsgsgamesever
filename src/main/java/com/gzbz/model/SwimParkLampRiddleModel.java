...
@ModelAnno(
   file = "SwimparkLampRiddle"
)
public class SwimParkLampRiddleModel extends BaseModel {
   @ModelColumnAnno(
      isPrimaty = true,
      comment = "序号"
   )
   ...
   public static void loadCustom(ModelPool pool, ConcurrentHashMap<?, ?> map) {
      ConcurrentHashMap<Integer, List<SwimParkLampRiddleModel>> stage_List = new ConcurrentHashMap();

      for(Map.Entry<?, ?> entry : map.entrySet()) {
         SwimParkLampRiddleModel value = (SwimParkLampRiddleModel)entry.getValue();
         ((List)stage_List.computeIfAbsent(value.getStage(), (k) -> new ArrayList())).add(value);
      }

      pool.putMap("CustomSwimparkLampRiddle", stage_List);
   }
