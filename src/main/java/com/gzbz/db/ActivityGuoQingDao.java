...
import com.gzbz.db.bean.GuoQingMonopolyCityEventData;
...
@DBTableAnno(
   name = "tb_activity_guo_qing",
   resultType = 0,
   selectKey = {"playerId"}
)
public class ActivityGuoQingDao extends AddUpAndConBaseDao {
   @DBColumnAnno(
      columnName = "playerId",
      isPrimary = true,
      comment = "玩家id"
   ...
   public Map<Integer, GuoQingCityBuildData> building = new HashMap();
   @DBColumnAnno(
      comment = "城主阁累计积分"
   )
   public int point;
   ...
   public GuoQingMonopolyCityEventData event = new GuoQingMonopolyCityEventData();
   ...
   public void loadBuilding(String data) {
      this.building = JsonUtil.jsonToMap(data, Integer.class, GuoQingCityBuildData.class);
   }
   ...
   public void loadEvent(String data) {
      this.event = (GuoQingMonopolyCityEventData)JsonUtil.jsonToBean(data, GuoQingMonopolyCityEventData.class);
   }
