...
public class ShopWarcraft extends AbstractShop {
   public ShopWarcraft(GamePlayer player) {
      super(player);
   }
   ...
   public ArrayList<ShopItemData> getShopList() {
      ...
      for(Map.Entry<Integer, Integer> entry : jsonData.map.entrySet()) {
         ...
         ShopWarcraftItemModel model = (ShopWarcraftItemModel)this.player.getGameModelPool().getEntity("shopWarcraftItem", id);
         if (model != null) {
   ...
   public boolean refresh() {
      ...
      int shopWarcraftFreeReflashNum = configManager.getIntDefault("shopWarcraftFreeReflashNum", 3);
      int shopWarcraftFreeReflashTime = configManager.getIntDefault("shopWarcraftFreeReflashTime", 28800);
      PlayerResetCustomCache playerResetCustomCache = (PlayerResetCustomCache)this.player.getCustomCache("tb_player_reset", this.player.getPlayerId());
      int resumeTime = (Integer)playerResetCustomCache.getForeverNum(142, 0);
      int curTime = DateUtil.getIntTime(System.currentTimeMillis());
      if (curTime < resumeTime && resumeTime - curTime >= shopWarcraftFreeReflashTime * (shopWarcraftFreeReflashNum - 1)) {
         int totalRefreshNum = (Integer)playerResetCustomCache.getDailyNum(141, 0);
         int shopWarcraftReflashNum = configManager.getIntDefault("shopWarcraftReflashNum", 20);
         if (totalRefreshNum >= shopWarcraftReflashNum) {
            this.player.failure(1000);
            return false;
         }

         int resourceType = Integer.parseInt(configManager.parseValue("shopWarcraftReflashCost", 0, "\\|"));
         int itemId = Integer.parseInt(configManager.parseValue("shopWarcraftReflashCost", 1, "\\|"));
         int itemNum = Integer.parseInt(configManager.parseValue("shopWarcraftReflashCost", 2, "\\|"));
         if (!this.player.checkResourceNum(resourceType, itemId, itemNum)) {
            this.player.failure(18);
            return false;
         }

         this.player.delResource(resourceType, itemId, (long)itemNum, 1, 102, 0, 0, "");
         playerResetCustomCache.addDailyReset(141, 1);
         ...
         int newResumeTime = beginTime + shopWarcraftFreeReflashTime;
         if (newResumeTime < curTime) {
            newResumeTime = curTime + shopWarcraftFreeReflashTime;
         }

         playerResetCustomCache.setForever(142, newResumeTime);
      ...
      if (!CollectionUtils.isEmpty(autoIdSet)) {
         for(ShopItemData shopItemData : this.getShopList()) {
            if (autoIdSet.contains(shopItemData.id)) {
               ShopWarcraftItemModel model = (ShopWarcraftItemModel)ApplicationContextProvider.getModelPoolEntity(this.getExcelName(), shopItemData.id);
   ...
   public void addExtraParams(ShopMsg.S2C_ShopData_7602.Builder builder) {
      ...
      int refreshTime = (Integer)playerResetCustomCache.getForeverNum(142, 0);
      int useFreeTime = 0;
      if (refreshTime != 0) {
         ...
         if (offset > 0) {
            ConfigManager configManager = (ConfigManager)ApplicationContextProvider.getContext().getBean(ConfigManager.class);
            int shopWarcraftFreeReflashTime = configManager.getIntDefault("shopWarcraftFreeReflashTime", 28800);
            useFreeTime = (offset - 1) / shopWarcraftFreeReflashTime + 1;
         }
      }
