...
//   }
    ...
    public void msgLogin(LoginMsg.C2S_Login_1000 msg, String source) {
        ...
        Channel channel = allChannel.get(source);
        if (channel == null) {
            logger.error("Login failed: source {} channel is null", source);
            return;
        }

        // ---------------- Server ID check ----------------
        if (!serversSet.contains(msg.getServerId())) {
            sendLoginResultAndClose(channel, LoginMsg.resultType.SERVERID);
            logger.error("Account {} login failed: invalid serverId", msg.getAccountName());
            return;
        }

        long nowTime = System.currentTimeMillis();

        // ---------------- Server open time check ----------------
        if (openServerTime > nowTime) {

            InetSocketAddress address = (InetSocketAddress) channel.remoteAddress();
            String ip = address.getAddress().getHostAddress();

            if (!gmMgr.PLATFORM_WHITE_LIST.contains(ip)) {

                Integer advanceEnter =
                        ApplicationContextProvider.getModelPoolEntity("customAdvanceEnterGame", country);

                if (advanceEnter == null ||
                        nowTime < openServerTime - advanceEnter * 1000L) {

                    sendLoginResultAndClose(channel, LoginMsg.resultType.SERVER_OPEN_TIME);
                    return;
                }
            }
        }
        ...
        // ---------------- Server full check ----------------
        int maxOnline = ApplicationContextProvider.getConfigInt("ServerFull", 5000);
        if (onlinePlayerMap.size() > maxOnline) {
            sendLoginResultAndClose(channel, LoginMsg.resultType.FULL);
            return;
        }
        ...
        GamePlayer gamePlayer =
                getGamePlayerByAccount(msg.getAccountName(), msg.getServerId());
        ...
        if (gamePlayer == null
                && isRegisterLimit()
                && !gmMgr.getAllAccountWhiteList().containsKey(msg.getAccountName())) {

            sendLoginResultAndClose(channel, LoginMsg.resultType.REGISTER_LIMIT);
            return;
        }

        // ---------------- Guest / Anti-addiction check ----------------
        if (isGuest) {

            LoginMsg.resultType guestCheck =
                    guestLimitTrigger(gamePlayer, msg.getDevUID());

            if (guestCheck != null) {
                sendLoginResultAndClose(channel, guestCheck);
                return;
            }

        } else if (msg.hasAge()) {

            LoginMsg.resultType antiAddiction =
                    antiAddictionTrigger(gamePlayer, msg.getAge());

            if (antiAddiction != null) {
                sendLoginResultAndClose(channel, antiAddiction);
                return;
            }
        }
        ...
        if (gamePlayer == null) {
            ...
            try {
                gameCachePool.insertDao(dao, true);
            } catch (Exception e) {
                sendLoginResultAndClose(channel, LoginMsg.resultType.KICKOUT);
                return;
            }

            GamePlayer newPlayer = context.getBean(GamePlayer.class);
            newPlayer.setChannel(channel);
            newPlayer.setPlayerIdAndAccount(dao.playerId, dao.account, dao.serverId);
            newPlayer.setDevUID(msg.getDevUID());
            newPlayer.setGuest(isGuest);
        ...
        if (!StringUtils.isEmpty(msg.getSign())) {
            ...
            if (StringUtils.isEmpty(msg.getAccountName())
                    || !msg.hasAccountType()
                    || !msg.hasLoginTime()
                    || msg.getLoginTime() <= 0
                    || StringUtils.isEmpty(msg.getChannelId())
                    || StringUtils.isEmpty(msg.getAccount())) {

                sendLoginResultAndClose(channel, LoginMsg.resultType.LOGIN_AUTH_FAIL);
                logger.error("SDK login verify failed: missing parameters");
                return;
            }

            String signStr = msg.getChannelId()
                    + msg.getAccount()
                    + msg.getAccountName()
                    + msg.getAccountType()
                    + msg.getLoginTime()
            ...
            String serverSign = MD5Util.encode(signStr);

            if (!serverSign.equals(msg.getSign())) {
                sendLoginResultAndClose(channel, LoginMsg.resultType.LOGIN_AUTH_FAIL);
                logger.error("SDK sign verify failed: serverSign={}, clientSign={}",
                        serverSign, msg.getSign());
                return;
            }
        }
        ...
        // -------- Reconnect handling (FIXED lambda issue) --------
        final byte isTempReConnect =
                (gamePlayer.getChannel() != null
                        && gamePlayer.getChannel() != channel) ? (byte) 1 : 0;

        if (isTempReConnect == 1) {

            channelMap.remove(gamePlayer.getChannel().id().asLongText());

            gamePlayer.getChannel()
                    .attr(AttributeKey.valueOf("playerId"))
                    .set(null);
        ...
        if (key == null) {
            ...
            if (playerDao.banEndTime > nowTime) {
                sendLoginResultAndClose(channel, LoginMsg.resultType.REFUSE);
                return;
            }

            // Internal account offline ban logic
            if (!Debug.isDebug
                    && playerDao.type == 998
                    && playerDao.logoutTime > 0L) {

                long offlineTime = nowTime - playerDao.logoutTime;
                long unsealInterval = nowTime - playerDao.unsealTime;
                long offlineDaysBan =
                        ApplicationContextProvider.getConfigInt("innterPlayerOfflineDaysBan", 7);

                if (offlineTime >= offlineDaysBan * 24L * 3600L * 1000L
                        && unsealInterval > 3600000L) {

                    gmMgr.gmBanRole(playerDao.playerId, 5256000);
                    sendLoginResultAndClose(channel, LoginMsg.resultType.REFUSE);
                    return;
                }
            }

            channel.attr(AttributeKey.valueOf("playerId"))
                    .set(gamePlayer.getPlayerId());

            gamePlayer.setChannel(channel);

            final byte reconnectFlag = isTempReConnect;
            ...
            channelMap.put(channel.id().asLongText(), gamePlayer);
            onlinePlayerMap.put(gamePlayer.getPlayerId(), gamePlayer);

            gamePlayer.pushTask(() -> gamePlayer.login(false));
        }
    }

    private void sendLoginResultAndClose(Channel channel,
                                         LoginMsg.resultType resultType) {

        LoginMsg.S2C_LoginResult_1001 protoMsg =
                LoginMsg.S2C_LoginResult_1001.newBuilder()
    ...
    public void setServerInfoToRedis(HashSet<Integer> slaveServerIds) {
        ...
        for (Map.Entry<Integer, ArrayList<String>> entry : (promoteWorldDao.jsonData).entrySet()) {
    ...
    public void createWarHero(int round) {
        ...
        worldDao.jsonData = new JingZhouData();
...
//   }
    public void initExeclModel() throws Exception {
        ...
        Map<Integer, ActivityRuleModel> activityRuleMap =
                ApplicationContextProvider.getModelPoolMap("activityRule");

        for (ActivityRuleModel rule : activityRuleMap.values()) {

            // Open type 3: custom date activity
            if (rule.getOpenType() == 3) {

                Map<Integer, ActivityOpenTimeModel> openTimeMap =
                        ApplicationContextProvider.getModelPoolEntity(
                                "customActivityOpenTime",
                ...
                int activeCount = 0;
                for (ActivityOpenTimeModel openTime : openTimeMap.values()) {
                    if (openTime.getIsClose() == 0) {
                        activeCount++;
                        if (activeCount > 1) {
                            throw new Exception(
                                    "Multiple open dates configured for activityId="
                                            + openTime.getActivityId()
                                            + " in activityOpenTime.xls"
                            );
                        }
                    }
                }
        ...
        // ================= Merge Activity Time Validation =================
        long mergeStartTime = DateUtil.getSomeDayBeginTime(this.mergeServerTime);
        int mergeOpenDays =
                ApplicationContextProvider.getConfigInt("mergeActivityOpenDays", 7);

        long mergeEndTime =
                mergeStartTime + mergeOpenDays * 86400000L - 1L;

        for (ActivityRuleModel rule : activityRuleMap.values()) {

            if (rule.getOpenType() == 7) {

                long subOpenTime =
                        mergeStartTime + rule.getOpenTime() * 86400000L;

                long subEndTime =
                        subOpenTime + rule.getDuration() * 1000L;

                boolean valid =
                        subOpenTime < subEndTime &&
                                subOpenTime >= mergeStartTime &&
                                subEndTime <= mergeEndTime;

                if (!valid) {
                    throw new Exception(
                            String.format(
                                    "Merge sub-activity %d time config invalid (out of merge range)",
                                    rule.getActivityType()
                            )
                    );
                }
        ...
        // ================= Mine Suggested Power =================
        Map<Integer, MineVeinModel> mineMap =
                ApplicationContextProvider.getModelPoolMap("mineVein");

        for (MineVeinModel model : mineMap.values()) {
            long power = monsterManager.getTotalPowerByBattleId(model.getBattleId());
            model.setSuggesPower(power);
        }

        // ================= Upgrade Cumulative Cost =================
        Map<Integer, UpgradeModel> upgradeModelMap =
                ApplicationContextProvider.getModelPoolMap("upgrade");
        ...
        if (upgradeModelMap != null) {
            for (Map.Entry<Integer, UpgradeModel> entry : upgradeModelMap.entrySet()) {
                ...
                if (totalExp > 0) {
                    costList.add(new ResourceModel(
                            1, PlayerDefine.PLAYER_HERO_EXP, totalExp));
                }

                if (totalCopper > 0) {
                    costList.add(new ResourceModel(
                            1, PlayerDefine.PLAYER_COPPER, totalCopper));
                }

                tempUpgradeMap.put(entry.getKey(), costList);

                totalExp += entry.getValue().getExpCost();
                totalCopper += entry.getValue().getSilverCost();
            }
        }

        this.upgradeMap = tempUpgradeMap;

        // ================= Hero Breach Cumulative Cost =================
        Map<Integer, List<ResourceModel>> tempBreachMap = new HashMap<>();
        int totalItem = 0;
        int totalBreachCopper = 0;

        for (int i = 0; i < 6; i++) {

            HeroBreachModel breachModel =
                    ApplicationContextProvider.getModelPoolEntity("heroBreach", i);

            totalItem += breachModel.getItemNum();
            totalBreachCopper += breachModel.getCopper();

            List<ResourceModel> list = new ArrayList<>();
            list.add(new ResourceModel(2, breachModel.getItemId(), totalItem));
            list.add(new ResourceModel(1, PlayerDefine.PLAYER_COPPER, totalBreachCopper));

            tempBreachMap.put(i + 1, list);
        }

        this.upBreachMap = tempBreachMap;

        // ================= Country Specific Turntable Task =================
        Map<Integer, TurntableActivityTaskModel> turntableMap =
                gameModelPool.getMap("krTurntableTask");

        if (turntableMap != null) {

            Map<Integer, TurntableActivityTaskModel> filtered =
                    new HashMap<>();

            for (TurntableActivityTaskModel model : turntableMap.values()) {

                if (country.equals(model.getAreanShow())) {
                    model.setId(model.getOldId());
                    filtered.put(model.getOldId(), model);
                }
            }

            turntableMap.clear();
            turntableMap.putAll(filtered);
        }

        // ================= Global Task Triggers =================
        GlobalTaskPart.initTriggers();

        // ================= Advance Enter Config =================
        ConcurrentHashMap<String, Integer> advanceEnterMap =
                new ConcurrentHashMap<>();

        String configStr =
                ApplicationContextProvider.getConfigString("advanceEnterGame", "tw:600");

        if (!StringUtils.isEmpty(configStr)) {

            for (String entry : configStr.split("\\|")) {
                String[] pair = entry.split(":");
                advanceEnterMap.put(pair[0], Integer.parseInt(pair[1]));
            }
        }

        gameModelPool.putMap("customAdvanceEnterGame", advanceEnterMap);

        // ================= Replace Configurations =================
        logger.info("Start replacing configuration...");

        replaceModel("skill", "skillNew");
        replaceModel("skillEffect", "skillEffectNew");
        replaceModel("eventHeroMain", "eventHeroMainNew");

        // ================= Country Specific Sign Award =================
        Map<Integer, FourteenTWSignAwardModel> signMap =
                gameModelPool.getMap("FourteenTWSignAward");

        if (signMap != null) {

            Map<Integer, FourteenTWSignAwardModel> filtered =
                    new HashMap<>();

            for (FourteenTWSignAwardModel model : signMap.values()) {
                if (country.equals(model.getAreanShow())) {
                    model.setId(model.getOldId());
                    filtered.put(model.getOldId(), model);
                }
            ...
            signMap.clear();
            signMap.putAll(filtered);
        }

        // ================= Country Specific Turntable Reward =================
        Map<Integer, TurntableRewardModel> drawMap =
                gameModelPool.getMap("krTurntableDraw");

        if (drawMap != null) {

            Map<Integer, TurntableRewardModel> filtered =
                    new HashMap<>();

            for (TurntableRewardModel model : drawMap.values()) {
                if (country.equals(model.getAreanShow())) {
                    model.setId(model.getOldId());
                    filtered.put(model.getOldId(), model);
                }
            ...
            drawMap.clear();
            drawMap.putAll(filtered);
        }

        logger.info("Configuration replacement completed.");
    }

    @SuppressWarnings("unchecked")
    private <T> void replaceModel(String oldKey, String newKey) {

        Map<Integer, T> oldMap = gameModelPool.getMap(oldKey);
        Map<Integer, T> newMap = gameModelPool.getMap(newKey);
        ...
        for (Map.Entry<Integer, T> entry : newMap.entrySet()) {
            ...
            if (oldMap.containsKey(id)) {
                oldMap.put(id, entry.getValue());
                logger.info("Replaced config [{}] id: {}", oldKey, id);
            }
        }
    }
...
//
//    }
public void initRobotAddHeroAndArraying(boolean isSync,
                                        RobotModel robotModel,
                                        PlayerDao playerDao) {

    GamePlayer robot = playerMap.get(playerDao.playerId);
    if (robot == null) {
        return;
    }

    HeroBagPart heroBagPart = robot.getMgrPart(HeroBagPart.class);

    Map<Integer, HeroDao> heroDaoMap =
            getGameCachePool().getMap("tb_hero_bag", new Object[]{robot.getPlayerId()});
    ...
    // ---------------- Collect old heroes to delete ----------------
    List<HeroDao> needDelHero = new ArrayList<>();
    if (heroDaoMap != null && !heroDaoMap.isEmpty()) {
        needDelHero.addAll(heroDaoMap.values());
    }

    List<HeroMsg.ArrayingInfo> arrayingList = new ArrayList<>();
    long totalPower = 0L;

    List<HeroRandomFun> heroConfigs = robotModel.getHeros();

    for (int i = 0; i < heroConfigs.size(); i++) {

        HeroRandomFun heroRandom = heroConfigs.get(i);

        if (heroRandom.getLv() <= 0) {
            continue;
        }

        int heroLv = heroRandom.getLv();
        int star = heroRandom.getStar();

        // ---------------- Resolve heroId ----------------
        int heroId;

        if (heroRandom.getId() > 0) {
            heroId = heroRandom.getId();
        } else {
            HeroModel randomHero =
                    monsterManager.getHeroModelByStarAndNataionAndType(
                            heroRandom.getStar(),
                            heroRandom.getNation(),
                            heroRandom.getType()
                    );
        ...
        HeroModel heroModel =
                gameModelPool.getEntity("hero", heroId);

        if (heroModel == null) {
            continue;
        }

        // ---------------- Limit level by max star ----------------
        ...
        if (heroRandom.getWearId() > 0) {

            RobotWearModel wearModel =
                    gameModelPool.getEntity("robotwear", heroRandom.getWearId());

            if (wearModel != null) {
                equips = monsterManager.getHeroEquip(wearModel);
                warriors = monsterManager.getHeroWarriors(wearModel);
                warcrafts = monsterManager.getHeroWarcrafts(wearModel);
            }
        }

        // ---------------- Calculate breach & star ----------------
        int breach = monsterManager.getBreachLvByLv(maxStar, heroLv);

        if (star == 0) {
            star = monsterManager.getStarByLv(heroId, heroLv);
        }

        // ---------------- Insert robot hero ----------------
        HeroDao heroDao = heroBagPart.insertRobotHero(
                heroId,
                heroLv,
                breach,
                star,
        ...
        // ---------------- Build arraying info ----------------
        HeroMsg.ArrayingInfo.Builder builder =
                HeroMsg.ArrayingInfo.newBuilder();

        builder.setCode(heroDao.code)
                .setPosition(i);

        if (i > 5) {
            builder.setCodeType(CommonMsg.CodeType.CODE_FRIEND);
        } else {
    ...
    // ---------------- Save arraying & update power ----------------
    if (arrayingList.isEmpty()) {
        return;
    }

    HeroArrayingPart heroArrayingPart =
            robot.getMgrPart(HeroArrayingPart.class);

    heroArrayingPart.saveHeroArraying(
            CommonMsg.HeroState.HERO_STATE_ARRAYING,
            arrayingList
    );

    heroArrayingPart.saveHeroArraying(
            CommonMsg.HeroState.HERO_STATE_ALLIANCE,
            arrayingList
    );
    ...
    if (playerDao.type == 2) {
        pvpSeasonMgr.insetNewPlayer(playerDao.playerId, robotModel.getScore());
    }

    playerDao.updateOp();

    final long finalTotalPower = totalPower;

    robot.pushTask(() -> {
        robot.getPlayerDao().combat_power = finalTotalPower;
        robot.getPlayerDao().updateOp();
    });
