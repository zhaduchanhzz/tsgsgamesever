...
@DBTableAnno(
   name = "tb_cross_activity"
)
public class CrossActivityDao extends DBDao {
   @DBColumnAnno(
      isPrimary = true,
      comment = "所属分区"
   )
   ...
   public void loadActivities(String data) {
      ...
      for(String activityData : dataArr) {
         ...
         for(String serverActivityData : serverActivityDataArr) {
            ...
            if (activityField.length > 3) {
               activityInfo.actOpenDay = Integer.parseInt(activityField[3]);
            }
   ...
   public String saveActivities() {
      ...
      for(Map.Entry<Integer, Map<Integer, ActivityInfo>> entry : this.activities.entrySet()) {
         ...
         for(Map.Entry<Integer, ActivityInfo> subEntry : (entry.getValue()).entrySet()) {
            ...
            stringBuffer.append(activityInfo.dropping).append("_");
            stringBuffer.append(activityInfo.actOpenDay);
            stringBuffer.append("|");
         }
